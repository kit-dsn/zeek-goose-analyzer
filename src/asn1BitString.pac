...
void getBoolsFromByte(ByteIterator & byte_it, ByteIterator & byte_end, VectorVal & vv, uint8 & current_byte, unsigned int & vector_it)
{
	for(; byte_it != byte_end; ++byte_it)
	{
		...
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x80 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x40 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x20 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x10 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x8 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x4 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x2 & current_byte));
		++vector_it;
		vv.Assign(vector_it, zeek::val_mgr->Bool(0x1 & current_byte));
		++vector_it;
		}
}
%}
...
%{
	...
	if(pad) {
		...
		{
			vv->Assign(vi, zeek::val_mgr->Bool(mask & current_byte));
			++vi;
			mask >>= 1;
		}
	}
