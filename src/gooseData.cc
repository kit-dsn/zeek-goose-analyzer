		...
		uint8 tag)
{
	...
	tmpDat->Assign(0, zeek::val_mgr->Count(tag)); 
		...
		const GOOSEData & binpacGooseData)
{
	switch(tag) {
		case BOOLEAN:
			recordGooseData->Assign(1,
				zeek::val_mgr->Bool(binpacGooseData.content()->boolean()));
			break;
		case BCD:
			recordGooseData->Assign(2,
				zeek::val_mgr->Int(binpacGooseData.content()->bcd()->val()));
			break;
		case SIGNED_INTEGER:
			recordGooseData->Assign(2,
				zeek::val_mgr->Int(binpacGooseData.content()->intVal()->val()));
			break;
		case UNSIGNED_INTEGER:
			recordGooseData->Assign(3,
				zeek::val_mgr->Count(binpacGooseData.content()->uintVal()->val()));
			break;
		case REAL:
			recordGooseData->Assign(4,
				zeek::make_intrusive<DoubleVal>(binpacGooseData.content()->realVal()->value()));
			break;
		case FLOATING_POINT:
			if(binpacGooseData.content()->floatVal()->formatSupported())
				recordGooseData->Assign(4,
					zeek::make_intrusive<DoubleVal>(binpacGooseData.content()->floatVal()->value()));
			break;
		case BIT_STRING:
			{
	...
	while(iteration_end_code(allDataIterator, allDataEnd, stackOfDataArrays, tmpDat))
	{
		...
		switch(tmpTag) {
			...
			default:
				...
				tmpDat->Assign(0, zeek::val_mgr->Count(tmpTag)); 
